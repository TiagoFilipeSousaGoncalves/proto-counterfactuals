# Source: https://github.com/cfchen-duke/ProtoPNet/blob/master/find_nearest.py
...
# Project Imports
from prototypes_utilities import compute_rf_prototype, find_high_activation_crop
...
def imsave_with_bbox(fname, img_rgb, bbox_height_start, bbox_height_end, bbox_width_start, bbox_width_end, color=(0, 255, 255)):
    ...
    # Load image in BGR format with OpenCV
...
# Function: Find the nearest patches in the dataset to each prototype
def find_k_nearest_patches_to_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
    ...
    for j in range(n_prototypes):
        ...
        if full_save:
            ...
            if not os.path.isdir(dir_for_saving_images):
                os.makedirs(dir_for_saving_images)
            ...
            for i, patch in enumerate(heaps[j]):
                ...
                imsave_with_bbox(
                    fname=os.path.join(dir_for_saving_images, 'nearest-' + str(i+1) + '_high_act_patch_in_original_img.png'),
    ...
    # print('Find nearest patches time: {0}'.format(time.time() - start))
...
# Function: Save prototype original image with bounding-box
