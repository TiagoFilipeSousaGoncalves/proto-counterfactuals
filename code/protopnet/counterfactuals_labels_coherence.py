...
for proto_stats_pr_df in proto_stats_df_list:
    ...
    for index, row in proto_stats_pr_df.iterrows():
        ...
        label_coherence_dict[row["Image"]]["Nearest Counterfactual Label"].append(nearest_counterfactual_label)
...
for image_filename in label_coherence_dict.keys():
    ...
    if len(label_coherence_dict[image_filename]["Nearest Counterfactual Label"]) == len(CHECKPOINTS):
        ...
        counterfactual_labels_among_models = np.array(counterfactual_labels_among_models)
...
if os.path.exists(os.path.join("results", DATASET.lower(), "protopnet", f"{FEATURE_SPACE.lower()}_cnt_label_coherence.txt")):
    os.remove(os.path.join("results", DATASET.lower(), "protopnet", f"{FEATURE_SPACE.lower()}_cnt_label_coherence.txt"))

report = open(os.path.join("results", DATASET.lower(), "protopnet", f"{FEATURE_SPACE.lower()}_cnt_label_coherence.txt"), "at")
