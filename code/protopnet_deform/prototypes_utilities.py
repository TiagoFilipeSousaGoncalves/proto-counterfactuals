...
# Function: Find high activation crop
...
def push_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
    ...
    for push_iter, (search_batch_input, search_y) in enumerate(dataloader):
        ...
        search_batch_input = search_batch_input.to(device)

        # This computation currently is not parallelized
        _, proto_act_torch = prototype_network_parallel.push_forward(search_batch_input)
    ...
    prototype_update = np.reshape(global_max_fmap_patches, tuple(prototype_network_parallel.prototype_shape))
    ...
    print('Duration of push operation: {0}'.format(end -  start))


    save_projected_prototype_images(
        prototype_network_parallel=prototype_network_parallel,
...
def update_prototypes_on_batch(search_batch_input,
    ...
    for j in range(n_prototypes):
        ...
        if class_specific:
            ...
            target_class = torch.argmax(prototype_network_parallel.prototype_class_identity[class_index]).item()
        ...
        if batch_max_proto_act_j > global_max_proto_act[j]:
            ...
            with torch.no_grad():
                ...
                batch_max_fmap_patch_j = get_deformed_patch(
                    input_normalized[img_index_in_batch, :, :, :].cpu(),
...
def save_projected_prototype_images(prototype_network_parallel,
    ...
    with torch.no_grad():
        ...
        for push_iter, (search_batch_input, search_y) in enumerate(dataloader):
            ...
            # This computation currently is not parallelized
            proto_in, proto_act_torch = prototype_network_parallel.push_forward(search_batch_input)
            proto_act_ = np.copy(proto_act_torch.detach().cpu().numpy())
            ...
            for j in range(n_prototypes):
                ...
                if image_indices[j, 0] > dataloader.batch_size * (push_iter + 1) or image_indices[j, 0] < dataloader.batch_size * push_iter:
                    continue
                ...
                if dir_for_saving_prototypes is not None:
                    ...
                    if prototype_img_filename_prefix is not None:
                        ...
                        offsets, _ = get_deformation_info(proto_in, prototype_network_parallel, device)
                        ...
                        for i in range(prototype_shape[-2]):
                            for k in range(prototype_shape[-1]):
                                ...
                                plt.imsave(
                                    os.path.join(dir_for_saving_prototypes,
...
# Function: Gets the offsets and normalized input for a given input
def get_deformation_info(protoL_input_, prototype_network_parallel, device):
    prototype_shape = prototype_network_parallel.prototype_shape
    epsilon_val = prototype_network_parallel.epsilon_val
    n_eps_channels = prototype_network_parallel.n_eps_channels
    ...
    input_vector_length = prototype_network_parallel.input_vector_length
    ...
    offsets = prototype_network_parallel.conv_offset(input_normalized).cpu()

    return offsets, input_normalized



# Function: Gets the deformed version of the input at the given location using the given offsets
def get_deformed_patch(input_, offsets, optimal_location, prototype_shape, dilation=(1,1), padding='nearest', padding_size=(1,1)):
    with torch.no_grad():
        result = np.zeros((prototype_shape[1], prototype_shape[2], prototype_shape[3]))
        n_eps_layers = int(input_.shape[-3] - prototype_shape[1])
        for i in range(prototype_shape[-2]):
            for j in range(prototype_shape[-1]):
                ...
                interpolated_input = interpolate_from_location(input_, (input_row, input_col), padding)
...
# Function: Location is (row, col) tuple
def interpolate_from_location(input_, location, padding='nearest'):
    ...
    # Shape of values is 4 x channels (one for each corner) ordered as top left, top right, bot left, bot right
    values = np.empty((4, input_.shape[-3]))
    values[:,:] = None


    # For now, just drop any offsetted bits that are out of bounds
    if padding == 'zero':
        if h_low < 0 or h_low > input_.shape[-1] - 1:
            values[0, :] = 0
            values[1, :] = 0
        if h_high > input_.shape[-1] - 1 or h_high < 0:
            values[2, :] = 0
            values[3, :] = 0
        if w_low < 0 or w_low > input_.shape[-2] - 1 :
            values[0, :] = 0
            values[2, :] = 0
        if w_high > input_.shape[-2] - 1 or w_high < 0:
            values[1, :] = 0
            values[3, :] = 0
        ...
        if h_low < 0:
            h_low = 0
        if h_high > input_.shape[-2] - 1:
            h_high = input_.shape[-2] - 1
        if w_low < 0:
            w_low = 0
        if w_high > input_.shape[-1] - 1:
            w_high = input_.shape[-1] - 1

    if np.isnan(values[0, :]).any():
        values[0, :] = input_[:, h_low, w_low]
    if np.isnan(values[1, :]).any():
        values[1, :] = input_[:, h_low, w_high]
    if np.isnan(values[2, :]).any():
        values[2, :] = input_[:, h_high, w_low]
    if np.isnan(values[3, 0]).any():
        values[3, :] = input_[:, h_high, w_high]
