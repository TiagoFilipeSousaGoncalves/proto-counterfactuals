# Source: https://github.com/jdonnelly36/Deformable-ProtoPNet/blob/main/model.py
...
# Dictionary that maps the name into backbone features
...
class PPNet(nn.Module):
    ...
    def __init__(
        ...
        deformable_conv_hidden_channels=0,
        ...
        # Here we are initializing the class identities of the prototypes w/out domain specific knowledge we allocate the same number of prototypes for each class
        ...
        # Prototypes (randomly initialised)
        ...
        # Last layer (MLP, for classification) - do not use bias
        self.last_layer = nn.Linear(self.num_prototypes, self.num_classes, bias=False)


        # Initialise layers' weights (if given)
        if init_weights:
            self._initialize_weights()


    # Method: Get convolutional features
    ...
    # Method: Cosine-activation for arc distance
    def cos_activation(self, x, is_train=True, prototypes_of_wrong_class=None):

        # Takes convolutional features and gives arc distance as in https://arxiv.org/pdf/1801.07698.pdf
        ...
        epsilon_channel_x = epsilon_channel_x.to(device=self.device)
        ...
        epsilon_channel_p = epsilon_channel_p.to(device=self.device)
        ...
        prototype_vector_length = prototype_vector_length.view(prototype_vector_length.size()[0], 1, prototype_vector_length.size()[1], prototype_vector_length.size()[2])
    ...
    # Method: Prototype Activations
    def prototype_activations(self, x, is_train=True, prototypes_of_wrong_class=None):
        # x is the raw input
        conv_features = self.conv_features(x)
        activations, marginless_activations = self.cos_activation(conv_features, is_train=is_train, prototypes_of_wrong_class=prototypes_of_wrong_class)
    ...
    def forward(self, x, is_train=True, prototypes_of_wrong_class=None):
        ...
        # Obtain prototype activations
        activations, additional_returns = self.prototype_activations(x, is_train=is_train, prototypes_of_wrong_class=prototypes_of_wrong_class)
    ...
    # Method:  Computes keypoint-wise orthogonality loss, ie encourage each piece of a prototype to be orthogonal to the others.
    # Inspired by https://openaccess.thecvf.com/content/ICCV2021/papers/Wang_Interpretable_Image_Recognition_by_Constructing_Transparent_Embedding_Space_ICCV_2021_paper.pdf
    def get_prototype_orthogonalities(self):
        ...
        prototype_piece_matrices = prototype_piece_matrices.transpose(2,3).reshape(self.num_prototypes_per_class, -1, self.prototype_shape[-3])
        ...
        orthogonalities -= torch.eye((self.num_prototypes // self.num_prototypes_per_class) * self.prototype_shape[-2] * self.prototype_shape[-1]).to(device=self.device)
        # num_protos * 9 * 9

        return orthogonalities


    # Method: Prune Prototypes
    def prune_prototypes(self, prototypes_to_prune):

        #  prototypes_to_prune: a list of indices each in [0, current number of prototypes - 1] that indicates the prototypes to be removed
    ...
    # Method: Get PPNet representation
    def __repr__(self):
    ...
    # Method: Set last layer incorrect connection
    ...
    # Method: Initialise offset weights
